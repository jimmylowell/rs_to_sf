SELECT
 ...
 ddl 
----------------------------------------------------------------------------
-- GET THIS DDL FOR SEQUENCE CREATION
-- FILTER ON "ddl LIKE '% DEFAULT %'"
--REGEXP_REPLACE(ddl, '(\\t)(sk_\\w*)( INTEGER NOT NULL DEFAULT ).*', 'CREATE OR REPLACE SEQUENCE ivr.seq_\\2;')
----------------------------------------------------------------------------
----------------------------------------------------------------------------
-- THIS REPLACE changes RS default TO SF default
--REGEXP_REPLACE(ddl, '(\\t)(sk_\\w*)( INTEGER NOT NULL DEFAULT ).*', '\\1\\2\\3 ivr.seq_\\2.nextval')
 ----------------------------------------------------------------------------
...
from (SELECT
 ...
 AND ddl NOT LIKE 'DISTSTYLE%'
 AND ddl NOT LIKE '%DISTKEY%'
 AND ddl NOT LIKE ',PRIMARY KEY%'
 AND ddl NOT LIKE 'ALTER TABLE%'
-------------------------------------------------------
--THIS FILTER TO CREATE sequence DDL
--AND ddl LIKE '% DEFAULT %'
-------------------------------------------------------
